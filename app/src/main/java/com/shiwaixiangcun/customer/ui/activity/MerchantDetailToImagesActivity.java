...
public class MerchantDetailToImagesActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        serlist = (List<MerchDetailBean.DataBean.AtlasBean>) intent.getSerializableExtra("serlist");
        layoutView();
        initData();
    }
    ...
    private void initData() {
        List<MerchDetailBean.DataBean.AtlasBean.AtlasListBean> atlasList = serlist.get(id).getAtlasList();
        if (serlist != null && serlist.size() != 0){
            ImageMerchantDetailAdapter imageDetailAdapter = new ImageMerchantDetailAdapter(atlasList,this);
            lv_image_detail.setAdapter(imageDetailAdapter);
        }
        String planName = serlist.get(id).getTitle();
